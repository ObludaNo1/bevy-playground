...
pub fn enemy_follow_player(
        ...
        enemy_query.iter_mut()
    {
        ...
        if distance > ai.detection_range {
            // Bevy’s change detection tracks when components are modified. If we set *state =
            // new_state every frame even when the value doesn’t change, Bevy thinks the state
            // changed. This triggers systems that run on Changed<CharacterState>, like animation
            // updates. By only updating when the value actually changes, we avoid unnecessary work.
